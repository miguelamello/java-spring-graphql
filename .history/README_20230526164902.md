...
```
Note: The structure of the type `Meter` and `Measurement` should mimic the structure of the database collections. In this case, the database tables are `meters` and `measurements`, respectively. The `readings` field in the `Meter` type is an array of `Measurement` objects. The `meter` field in the `Measurement` type is a `Meter` object.
