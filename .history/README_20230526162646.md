...
Returns an array of measurements. Each measurement object contains the `id`, `meter`, `value` and `timestamp` of the reading. Note: Here `meter` field is an object that contains the meter `id`, `name`, `location`, `created`, `updated`.
