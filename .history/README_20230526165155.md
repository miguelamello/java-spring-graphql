...
```
Note: The structure of the type `Meter` and `Measurement` should mimic the structure of the database collections. In this case, the collection are `meters` and `measurements`, respectively. The `readings` field in the `Meter` type is an array of `Measurement` objects. The `meter` field in the `Measurement` type is a `Meter` object. Both are used only for querying purposes. As you can see, you can have nested objects in GraphQL that not necessarily are part of the database schema. For detailed information about the GraphQL Schema Definition Language (SDL), check out the [official documentation](https://graphql.org/learn/schema/).
